...
def draw():
    ...
    with ui.row():
        ...
        with ui.card():
            ui.label("sort2")
            sort2 = sortable.Column(value=list2.value, class_obj=Custom, group="a")

        with ui.card().classes("bg-red-100"):  # .classes("size-24 h-fit min-h-24"):
            ui.label("sort3")
            sort3 = sortable.Column(
                value=list3.value, class_obj=Custom, align_items="center"
            ).classes("w-full h-fit min-h-12 p-1")

    ui.label("sort1 & sort2 are in same group, while sort3 is not.")
    ...
    ui.label("Pop/Insert via function")
